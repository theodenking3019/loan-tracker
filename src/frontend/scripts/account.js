...
window.onload = async function () {
    try {
        ...
        document.getElementById('userEthAddress').textContent =
            data.userEthereumAddress;

        if (outstanding && outstanding > 0) {
            document.getElementById('borrowFormContainer').style.display =
                'none'; // Hide the borrow form
            document.getElementById('repaymentFormContainer').style.display =
                'block'; // Show the repayment form
            document.getElementById('outstandingBalance').textContent =
                principal; // Display the outstanding balance
            document.getElementById('totalLoanAmount').textContent =
                outstanding; // Display the outstanding balance
            document.getElementById('repaymentAmount').focus();
        } else {
            document.getElementById('borrowFormContainer').style.display =
                'block'; // Hide the borrow form
            document.getElementById('repaymentFormContainer').style.display =
                'none'; // Show the repayment form
